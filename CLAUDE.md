...
| `guest_user`                    | Second user with role "guest"                 |
| `sample_team`                   | Bare Team with no members                     |
| `sample_team_with_owner_member` | Team + sample_user as owner                   |
| `team_with_community_plan`      | Team + community billing plan                 |
| `team_with_business_plan`       | Team + active business subscription           |
| `authenticated_api_client`      | (Client, AccessToken) tuple for API tests     |
...
Entity primary keys are 12-character alphanumeric tokens generated by `generate_id()` from `sbomify.apps.core.utils`, not auto-incrementing integers.
...
```python
htmx_success_response(message, triggers=None)  # toast + optional HTMX triggers
htmx_error_response(message, triggers=None)     # error toast + HX-Reswap: none
htmx_error_from_exception(error: DomainError)   # converts DomainError to HTMX response
```
...
Three roles: `"owner"`, `"admin"`, `"guest"`. CBV mixins in `sbomify.apps.teams.permissions`:
