...
def list_products(request: HttpRequest):
    ...
    try:
        products = Product.objects.filter(team_id=team_id).prefetch_related("projects", "identifiers")
...
def get_product(request: HttpRequest, product_id: str):
    ...
    try:
        product = Product.objects.prefetch_related("projects", "identifiers").get(pk=product_id)
