...
class PublicStatusToggleManager {
  ...
  private initialize(): void {
    ...
    // Public URL links are now simple links - no special handling needed
  }
  ...
  private updatePublicUrlVisibility(isPublic: boolean): void {
    ...
    if (isPublic) {
      ...
      // Refresh any copyable value components in the public URL info
      const copyableElements = this.publicUrlInfo.querySelectorAll('[data-copyable-value]');
      copyableElements.forEach(el => {
        // Trigger any copy functionality initialization if needed
        const event = new CustomEvent('copyable-refresh');
  ...
  private setLoadingState(loading: boolean): void {
    if (this.checkbox) {
      ...
      // Removed attachPublicUrlListener - using simple links now
...
document.addEventListener('DOMContentLoaded', () => {
  const toggleContainers = document.querySelectorAll('.public-status-toggle');

  toggleContainers.forEach(container => {
    const form = container.querySelector('form') as HTMLFormElement;
    if (form) {
      console.log('Initializing PublicStatusToggle for server-side form');
      new PublicStatusToggleManager(form);
    }
  });
});

export { PublicStatusToggleManager };
