...
| The severity and status colours were one value for both themes, and each is read as text over a 12% tint of itself. On the white surface that tint is almost white: Medium measured 1.53:1 against the 4.5 WCAG AA needs. `test_component_colours.py` was green throughout, because it asserts the rendered ink equals the palette token and the token was the same in both themes. | A colour used as a fill and a colour used as ink on that fill cannot be the same value on both a white and a near-black surface. Give the theme its own value, and check the result rather than the token: matching the palette says nothing about whether the palette works. Decorative `aria-hidden` marks are exempt and keep the accent. | [Palette](../sbomify/assets/css/tailwind.src.css), [which token a component reaches for](../sbomify/apps/core/tests/e2e/test_component_colours.py), [whether it is readable](../sbomify/apps/core/tests/e2e/test_component_contrast.py). |
...
| The migration promised 44px touch targets. The top bar kept it; the controls inside the page did not, so row action menus were 36px, inventory filters 35px and pagination 32px on a phone. | A shared control states its own phone floor, in the size segment where the rest of its dimensions live, as `max-sm:min-h-11` so it never competes with the height it overrides. Measure the target that answers a tap, not the border box: `c-forms.checkbox-input` is 20px and already reaches 44px through an invisible `::after`, which a `getBoundingClientRect` check reports as a failure it is not. | [Icon button](../sbomify/templates/components/buttons/icon.html), [button](../sbomify/templates/components/buttons/button.html), [select](../sbomify/templates/components/forms/select.html), [pager cell](../sbomify/templates/components/navigation/page_link.html), [mobile layout](../sbomify/apps/core/tests/e2e/test_mobile_layout.py). |
