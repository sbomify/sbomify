...
def _build_common_context(assessment: CRAAssessment) -> dict[str, Any]:
    ...
    return {
        ...
        "manufacturer_name": manufacturer.name if manufacturer else "[Manufacturer Name]",
        "manufacturer_address": manufacturer.address if manufacturer else "",
        "manufacturer_email": manufacturer.email if manufacturer else "",
        "manufacturer_website": (manufacturer.website_urls[0] if manufacturer and manufacturer.website_urls else ""),
        "security_contact_email": security_contact.email if security_contact else "",
...
def _build_security_txt_context(assessment: CRAAssessment, base: dict[str, Any]) -> dict[str, Any]:
    ...
    for country in assessment.target_eu_markets:
        for lang in _COUNTRY_LANGUAGE_MAP.get(country, "").split(","):
            if lang:
                languages.add(lang)
...
def _build_document_context(assessment: CRAAssessment, kind: str) -> dict[str, Any]:
    ...
    if kind == CRAGeneratedDocument.DocumentKind.USER_INSTRUCTIONS:
        base["update_frequency"] = assessment.update_frequency
        base["update_method"] = assessment.update_method
        base["update_channel_url"] = assessment.update_channel_url
        base["support_email"] = assessment.support_email
        base["support_url"] = assessment.support_url
        base["support_phone"] = assessment.support_phone
        base["support_hours"] = assessment.support_hours
        base["data_deletion_instructions"] = assessment.data_deletion_instructions
        return base
...
def regenerate_all(assessment: CRAAssessment) -> ServiceResult[int]:
    """Regenerate all document kinds. Returns count."""
    count = 0
