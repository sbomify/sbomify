...
def list_cle_events(request: HttpRequest, product_id: str) -> Any:
...
def create_cle_event_endpoint(request: HttpRequest, product_id: str, payload: CLEEventCreateSchema) -> Any:
    ...
    if not svc_result.ok:
        ...
        return status, {"detail": svc_result.error, "error_code": ErrorCode.BAD_REQUEST}
...
def list_cle_support_definitions(request: HttpRequest, product_id: str) -> Any:
