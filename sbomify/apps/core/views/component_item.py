    ...
    def get(self, request: HttpRequest, component_id: str, item_type: str, item_id: str) -> HttpResponse:
        ...
        if is_sbom_backed:
            ...
            try:
                ...
                # The card reads counts from each run's summary and exactly one
                # title, and it loops `latest_runs` alone. Asking for the whole
                # response built every finding of every run twice, which is how
                # this page reached 31 MB and a 504 on a four-thousand-finding
                # scan.
