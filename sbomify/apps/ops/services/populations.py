...
def people() -> QuerySet[User]:
    ...
    Excludes two populations the raw ``User`` table carries:
    ...
    * Synthetic OIDC bot identities, which are publishing credentials wearing
    ...
    """
    return User.objects.filter(deleted_at__isnull=True).exclude(_IS_BOT)
...
def artifacts() -> QuerySet[SBOM]:
    """Every uploaded BOM row, of any ``bom_type``.

    Named artifacts rather than SBOMs because that is what it is: the table
    holds CBOMs, HBOMs, AI BOMs, VEX documents and more, and the glossary
    reserves "SBOM" for the case where the type is the point. The old
    dashboard labelled this "Total SBOMs".
    """
    return SBOM.objects.all()


def sboms() -> QuerySet[SBOM]:
    """Artifacts that really are SBOMs."""
