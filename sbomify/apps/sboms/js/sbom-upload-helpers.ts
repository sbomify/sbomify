...
export type UploadBomType = 'sbom' | 'vex' | 'cbom'

export function bomTypeLabel(bomType: UploadBomType): string {
    if (bomType === 'vex') return 'VEX'
    if (bomType === 'cbom') return 'CBOM'
    return 'SBOM'
}
...
export function validateUploadFile(file: File, bomType: UploadBomType): string | null {
    ...
    if (!hasValidType && !hasValidExtension) {
        const allowed = bomType === 'vex' ? '.json, .cdx, .xml' : bomType === 'cbom' ? '.json, .cdx' : '.json, .spdx, .cdx'
        return `Please select a valid ${bomTypeLabel(bomType)} file (${allowed})`
    }

    // XML is only meaningful for CycloneDX VEX; SBOM and CBOM uploads are
    // JSON formats. Catch by extension AND by MIME (a .cdx file the browser
    // tags as XML).
    const isXml = fileExtension === '.xml' || file.type === 'application/xml' || file.type === 'text/xml'
    if (bomType !== 'vex' && isXml) {
        return `XML uploads are supported for VEX only; a ${bomTypeLabel(bomType)} must be JSON`
    }
    ...
    if (bomType === 'cbom' && /\.spdx(\.|$)/.test(file.name.toLowerCase())) {
        return 'A CBOM must be a CycloneDX JSON document (.json, .cdx)'
    }
