...
class TestStripPurlQualifiers:
    ...
    def test_strip_qualifiers_simple(self):
        assert strip_purl_qualifiers("pkg:pypi/requests?arch=arm64") == "pkg:pypi/requests"

    def test_strip_qualifiers_with_version(self):
        assert strip_purl_qualifiers("pkg:pypi/requests@1.0.0?arch=arm64") == "pkg:pypi/requests@1.0.0"

    def test_strip_qualifiers_and_subpath(self):
        assert strip_purl_qualifiers("pkg:pypi/lib@1.0?ext=whl#sub") == "pkg:pypi/lib@1.0"

    def test_strip_subpath_only(self):
        assert strip_purl_qualifiers("pkg:github/sbomify/sbomify@v1.0#src") == "pkg:github/sbomify/sbomify@v1.0"

    def test_no_qualifiers_unchanged(self):
        assert strip_purl_qualifiers("pkg:pypi/requests") == "pkg:pypi/requests"
...
@pytest.mark.django_db
class TestTeaTeiMapperPurlQualifierFallback:
    """Tests for PURL qualifier fallback matching in TEI discovery."""

    def test_qualified_query_matches_unqualified_stored(self, tea_enabled_product):
        """Query with qualifiers should match stored bare PURL via fallback."""
        ProductIdentifier.objects.create(
            product=tea_enabled_product,
            team=tea_enabled_product.team,
            identifier_type=ProductIdentifier.IdentifierType.PURL,
    ...
    def test_unqualified_query_matches_qualified_stored(self, tea_enabled_product):
        """Query without qualifiers should match stored qualified PURL via fallback."""
        ProductIdentifier.objects.create(
            product=tea_enabled_product,
            team=tea_enabled_product.team,
            identifier_type=ProductIdentifier.IdentifierType.PURL,
    ...
    def test_different_qualifiers_match_via_fallback(self, tea_enabled_product):
        """Query with different qualifiers should still match via base PURL fallback."""
        ProductIdentifier.objects.create(
            product=tea_enabled_product,
            team=tea_enabled_product.team,
            identifier_type=ProductIdentifier.IdentifierType.PURL,
            value="pkg:pypi/test-package?os=linux",
        )
        release = Release.objects.create(product=tea_enabled_product, name="v1.0.0")
    ...
    def test_qualified_query_with_version_matches_unqualified_stored(self, tea_enabled_product):
        ...
        tei = "urn:tei:purl:example.com:pkg:pypi/test-package@1.0.0?arch=arm64"
        releases = tea_tei_mapper(tea_enabled_product.team, tei)
    ...
    def test_no_false_positive_on_name_prefix(self, tea_enabled_product):
        ...
        ProductIdentifier.objects.create(
            ...
            team=tea_enabled_product.team,
            identifier_type=ProductIdentifier.IdentifierType.PURL,
            value="pkg:pypi/test-package-extra",
        )
