...
from sbomify.apps.teams.models import ContactProfile, CustomDomain, Member, Team
...
def _check_custom_domain_access(team: Team) -> tuple[bool, str | None]:
    """Check if team has access to custom domain feature.

    Custom domains are only available for business and enterprise plans.

    Returns:
        Tuple of (has_access, error_message)
    """
    if not team.billing_plan:
        return False, "Custom domains require a business or enterprise plan"

    try:
        plan = BillingPlan.objects.get(key=team.billing_plan)
        if plan.key not in ["business", "enterprise"]:
            return False, "Custom domains are only available for business and enterprise plans"
...
def _validate_hostname(hostname: str) -> tuple[bool, str | None]:
    ...
    if " " in hostname or hostname.startswith("http://") or hostname.startswith("https://"):
        return False, "Hostname must be a valid domain name without protocol"

    # Basic domain validation regex
    # Allows: alphanumeric, dots, hyphens. No leading/trailing hyphens.
...
@router.post(
    ...
    response={201: CustomDomainSchema, 400: ErrorResponse, 403: ErrorResponse, 404: ErrorResponse},
)
def create_custom_domain(request: HttpRequest, team_key: str, payload: CustomDomainCreateSchema):
    """Create a custom domain for the workspace.

    Only business and enterprise plans can create custom domains.
    Each workspace can only have one custom domain.
    """
    try:
        team_id = token_to_number(team_key)
    except ValueError:
    ...
    if not member or member.role not in ["owner", "admin"]:
        return 403, {"detail": "Only owners and admins can manage custom domains"}

    # Check if team has access to custom domain feature
    has_access, error_msg = _check_custom_domain_access(team)
    if not has_access:
        return 403, {"detail": error_msg}

    # Validate hostname
    is_valid, error_msg = _validate_hostname(payload.hostname)
    if not is_valid:
        return 400, {"detail": error_msg}

    # Check if team already has a custom domain
    if hasattr(team, "custom_domain") and team.custom_domain:
        return 400, {"detail": "Workspace already has a custom domain. Please delete the existing one first."}

    # Check if hostname is already taken
    if CustomDomain.objects.filter(hostname=payload.hostname).exists():
        return 400, {"detail": "This hostname is already in use"}
    ...
    try:
        ...
        logger.error(f"Error creating custom domain for team {team_key}: {e}")
        return 400, {"detail": "Failed to create custom domain"}


@router.get(
    "/{team_key}/custom-domain",
    response={200: CustomDomainSchema, 403: ErrorResponse, 404: ErrorResponse},
)
def get_custom_domain(request: HttpRequest, team_key: str):
    ...
    try:
        ...
        return 404, {"detail": "Custom domain not found for this workspace"}


@router.patch(
    "/{team_key}/custom-domain",
    response={200: CustomDomainSchema, 400: ErrorResponse, 403: ErrorResponse, 404: ErrorResponse},
)
def update_custom_domain(request: HttpRequest, team_key: str, payload: CustomDomainUpdateSchema):
    """Update the workspace's custom domain.

    Only is_active can be updated. Hostname cannot be changed via PATCH.
    """
    try:
        team_id = token_to_number(team_key)
    except ValueError:
    ...
    if payload.is_active is not None:
        custom_domain.is_active = payload.is_active
...
def delete_custom_domain(request: HttpRequest, team_key: str):
    """Delete the workspace's custom domain."""
    try:
        team_id = token_to_number(team_key)
    except ValueError:
        return 404, {"detail": "Team not found"}
...
@router.post(
    "/{team_key}/custom-domain/verify",
    response={202: CustomDomainSchema, 403: ErrorResponse, 404: ErrorResponse},
)
def verify_custom_domain(request: HttpRequest, team_key: str):
    """Verify the workspace's custom domain via DNS check.

    Triggers an async task to check if the domain resolves to the configured EDGE_IPS.
    Returns 202 Accepted to indicate the verification task has been queued.
    """
    try:
        team_id = token_to_number(team_key)
    except ValueError:
    ...
    member = Member.objects.filter(user=request.user, team=team).first()
    if not member or member.role not in ["owner", "admin"]:
        return 403, {"detail": "Only owners and admins can verify custom domains"}
    ...
    # Trigger async DNS verification task
    from sbomify.apps.teams.tasks import verify_custom_domain_task

    verify_custom_domain_task.send(custom_domain.id)

    logger.info(f"Triggered DNS verification task for domain {custom_domain.hostname} (team: {team.key})")

    # Return current state (verification in progress)
    return 202, CustomDomainSchema(
        id=custom_domain.id,
        hostname=custom_domain.hostname,
        is_verified=custom_domain.is_verified,
        is_active=custom_domain.is_active,
