...
def test_invite_limit_counts_pending_invitations(django_user_model):
    ...
    BillingPlan.objects.create(key="community", name="Community", description="Community Plan", max_users=2)
    ...
    Invitation.objects.create(
        team=team, email="invitee@example.com", role="admin", expires_at=timezone.now() + timezone.timedelta(days=7)
    )
