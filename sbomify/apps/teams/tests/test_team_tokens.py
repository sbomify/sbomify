...
class TestTeamTokensView:
    ...
    def test_get_requires_team_membership(
    ...
    def test_get_renders_template(
        ...
        assert b"Personal Access Tokens" in response.content or b"Access Tokens" in response.content

    def test_get_shows_existing_tokens(
        self, client: Client, sample_team_with_owner_member
    ):
        """Test that GET shows existing access tokens."""
        team = sample_team_with_owner_member.team
        user = sample_team_with_owner_member.user
        setup_authenticated_client_session(client, team, user)
        ...
        response = client.get(reverse("teams:team_tokens", kwargs={"team_key": team.key}))
        assert response.status_code == 200
        assert b"Test Token" in response.content

    def test_post_creates_token(
        self, client: Client, sample_team_with_owner_member
    ):
        """Test that POST creates a new access token."""
        team = sample_team_with_owner_member.team
        user = sample_team_with_owner_member.user
        setup_authenticated_client_session(client, team, user)
    ...
    def test_post_invalid_form_returns_error(
        self, client: Client, sample_team_with_owner_member
    ):
    ...
    def test_member_role_can_access(
        self, client: Client, sample_team_with_owner_member
    ):
        ...
        member_membership = team.member_set.exclude(user=owner).first()
    ...
    def test_admin_role_can_access(
        self, client: Client, sample_team_with_owner_member
    ):
        ...
        admin_membership = team.member_set.exclude(user=owner).first()
    ...
    def test_guest_role_cannot_access(
        self, client: Client, sample_team_with_owner_member
    ):
        ...
        guest_membership = team.member_set.exclude(user=owner).first()
        if not guest_membership:
            from django.contrib.auth import get_user_model

            User = get_user_model()
            guest_user = User.objects.create_user(
                username="guest", email="guest@example.com", password="test"
            )
            Member.objects.create(team=team, user=guest_user, role="guest")
        else:
            guest_user = guest_membership.user
