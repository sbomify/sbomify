...
def verify_custom_domain_dns(hostname: str) -> bool:
    """Verify that a hostname has a CNAME record pointing to APP_BASE_URL domain.

    Checks if the domain has a CNAME record pointing to the domain from APP_BASE_URL.
    For example, if APP_BASE_URL is "https://sbomify.example.com", the customer's
    domain should have a CNAME pointing to "sbomify.example.com".

    Args:
        hostname: The fully qualified domain name to verify (e.g., trust.example.com)

    Returns:
        True if the domain has a valid CNAME pointing to APP_BASE_URL domain, False otherwise
    """
    try:
        ...
        target_domain = urlparse(app_base_url).netloc

        if not target_domain:
            logger.error(f"Could not extract domain from APP_BASE_URL: {app_base_url}")
            return False

        logger.info(f"Verifying CNAME for {hostname} -> expected target: {target_domain}")
        ...
        try:
            ...
            for rdata in cname_answers:
                ...
                logger.info(f"Found CNAME: {hostname} -> {cname_target}")

                # Check if CNAME points to target domain
                if cname_target == target_domain:
                    logger.info(f"DNS verification successful: {hostname} CNAME points to {target_domain}")
                    return True
                else:
                    logger.warning(
                        f"CNAME verification failed: {hostname} points to {cname_target}, expected {target_domain}"
                    )
                    return False

            # Should not reach here, but handle empty response
            logger.warning(f"CNAME query returned empty results for {hostname}")
            return False

        except dns.resolver.NoAnswer:
            logger.warning(f"No CNAME record found for {hostname}")
            return False
        ...
        logger.error("dnspython library not installed - cannot verify DNS")
        return False
    except Exception as e:
        logger.error(f"Error verifying custom domain DNS for {hostname}: {e}", exc_info=True)
        return False
